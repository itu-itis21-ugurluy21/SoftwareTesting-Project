...
class TestDoAlgebra(unittest.TestCase):
    ...
    def test_addition_minimal_input(self):
        """1 + 2 → 3 (minimum-length expression)"""
        self.assertEqual(do_algebra(['+'], [1, 2]), 3)

    def test_mixed_precedence_add_mul_sub(self):
        """2 + 3*4 – 5 → 9 (addition, multiplication, subtraction)"""
        self.assertEqual(do_algebra(['+', '*', '-'], [2, 3, 4, 5]), 9)

    def test_exponentiation_then_floor_div_then_add(self):
        """2**3 // 20 + 3 → 3 (checks ** before // before +)"""
        self.assertEqual(do_algebra(['**', '//', '+'], [2, 3, 20, 3]), 3)

    def test_multiplication_by_zero_inside_chain(self):
        """10 – 0*3 + 2 → 12 (zero with higher-precedence *)"""
        self.assertEqual(do_algebra(['-', '*', '+'], [10, 0, 3, 2]), 12)

    def test_all_precedence_tiers_together(self):
        """7*2 + 5//3 – 4 → 11 (full operator spread)"""
        self.assertEqual(do_algebra(['*', '+', '//', '-'], [7, 2, 5, 3, 4]), 11)
...
class TestNumericalLetterGrade(unittest.TestCase):
    ...
    def test_invalid_values_raise(self):
        """GPAs < 0 or > 4 should raise ValueError (per implementation)."""
        with self.assertRaises(ValueError):
...
class TestOddCount(unittest.TestCase):
    ...
    def test_odd_count_zero(self):
        self.assertEqual(odd_count(['02468']),['the number of odd elements 0n the str0ng 0 of the 0nput.'])
